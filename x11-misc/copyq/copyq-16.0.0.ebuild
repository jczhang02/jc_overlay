...
IUSE="crypt gnome keyring notification sound test X"

REQUIRED_USE="keyring? ( crypt )"
RESTRICT="!test? ( test )"
...
RDEPEND="
	...
	keyring? ( dev-libs/qtkeychain[keyring] )
	X? (
		dev-qt/qtbase:6=[X]
		x11-libs/libX11
		x11-libs/libXfixes
...
src_configure() {
	local mycmakeargs=(
		...
		-DWITH_GNOME_CLIPBOARD_EXTENSION=$(usex gnome)
		-DWITH_KEYCHAIN=$(usex keyring)
		-DWITH_NATIVE_NOTIFICATIONS=$(usex notification)
		-DWITH_QCA_ENCRYPTION=$(usex crypt)
		-DWITH_TESTS=$(usex test)
		-DWITH_X11=$(usex X)
	)

	use sound && mycmakeargs+=( -DMINIAUDIO_INCLUDE_DIR="${EPREFIX}/usr/include/miniaudio" )
...
pkg_postinst() {
	xdg_pkg_postinst
	optfeature "encryption support in commands" app-crypt/gnupg
}
