...
# Local backports (neither upstream as of 1.15.11):
#   1. PR #25886 — retry OpenAI overload stream errors (server_is_overloaded /
#      service_unavailable_error) instead of terminating the turn with a final
#      "OpenAI Responses stream error".
#   2. ephemeral tool-result attachments — keep Read-tool media (PDFs/images)
#      only for the current turn; strip from older history so large base64
#      blobs are not re-sent on every request (the other cause of recurring
#      "OpenAI Responses stream error": oversized payload, ~60s, 0 tokens).
PATCHES=(
	"${FILESDIR}/${PN}-1.15.11-retry-overload-25886.patch"
	"${FILESDIR}/${PN}-1.15.11-ephemeral-attachments.patch"
)
...
pkg_postinst() {
	...
	elog
	elog "Local backports applied (neither upstream as of 1.15.11):"
	elog "  - PR #25886: retry OpenAI overload stream errors"
	elog "  - ephemeral tool-result attachments: large Read-tool PDFs/images are"
	elog "    no longer re-sent every request, only kept for the current turn."
}
