...
KEYWORDS="-* ~amd64"
# Upstream only ships/targets glibc x86_64 for Linux.
...
src_compile() {
	...
	einfo "Installing dependencies (bun install)"
	cd "${S}" || die
	# Upstream build-linux.sh intentionally uses a non-frozen install. The
	# release tarball's lockfile is regenerated by Bun during resolution.
	# Use the public npm registry and bypass stale manifest metadata; otherwise
	# Bun can incorrectly report existing versions as unavailable. Limit network
	# concurrency and retry because Bun can occasionally leave large npm tarballs
	# truncated, which then causes IntegrityCheckFailed during extraction.
	local attempt
	for attempt in 1 2 3; do
		if bun install --registry="${npm_config_registry}" --no-cache \
