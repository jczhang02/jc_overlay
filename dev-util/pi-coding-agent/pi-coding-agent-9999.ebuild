...
EGIT_REPO_URI="https://github.com/earendil-works/pi.git"
EGIT_BRANCH="main"
...
src_unpack() {
	...
	# The git checkout is already staged in ${S}; unpack the selected bun
	# archive separately under ${T} so it does not pollute the source tree.
	local bun_zip
	case ${ARCH} in
...
pkg_postinst() {
	elog "pi live checkout installed as /opt/bin/pi (built from source via bun --compile)."
	elog
