...
pkg_nofetch() {
	einfo "ToDesk server requires a User-Agent header to serve the actual deb package."
	einfo "Please download manually with:"
	einfo "  wget -U 'Mozilla/5.0' -O '/var/cache/distfiles/${MY_P}-amd64.deb' 'https://dl.todesk.com/linux/${MY_P}-amd64.deb'"
}

src_unpack() {
	local deb_file="${MY_P}-amd64.deb"
	local deb_path="${DISTDIR}/${deb_file}"
	local distfiles_path="/var/cache/distfiles/${deb_file}"

	# Check in Portage DISTDIR first, then in /var/cache/distfiles
	if [[ ! -f "${deb_path}" ]] && [[ -f "${distfiles_path}" ]]; then
		deb_path="${distfiles_path}"
	fi

	if [[ ! -f "${deb_path}" ]]; then
		die "File not found. Please download to /var/cache/distfiles/ with: wget -U 'Mozilla/5.0' -O '/var/cache/distfiles/${deb_file}' 'https://dl.todesk.com/linux/${deb_file}'"
	fi
